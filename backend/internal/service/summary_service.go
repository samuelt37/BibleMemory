...
func (s *SummaryService) gradeAllWithAI(userAnswers, passages, notesContext []string) ([]dto.SummaryResult, error) {
	...
	sb.WriteString("- If notes were present AND a bonus point was actually applied (base score was 4+), you may mention that\n")
	sb.WriteString("  notes are available for this passage and factored into the score.\n")
	sb.WriteString("- If notes were present but NO bonus was applied (base score was 3 or lower), you may still note that\n")
	sb.WriteString("  notes are available for this passage, but do NOT say they factored into the score — instead encourage\n")
	sb.WriteString("  the user to expand their summary to better reflect both the passage and their own notes.\n")
	sb.WriteString("- If no notes are provided, base the feedback solely on the passage and summary, with no mention of notes.\n\n")
